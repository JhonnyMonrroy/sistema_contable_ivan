...
class AllegedRC4 {
     ...
     */
    static function encryptMessageRC4($mensaje, $key,$unscripted=false){
        $state = range(0, 255);
        $x=0;
        $y=0;
        $index1=0;
        ...
        for($i=0; $i<strlen($mensaje);$i++ ){
            ...
            $mensajeencriptado = $mensajeencriptado . (($unscripted)?"":"-"). ((strlen($nmenHex)==1)?('0'.$nmenHex):$nmenHex);            
        }        
        return (($unscripted)?$mensajeencriptado:substr($mensajeencriptado,1,strlen($mensajeencriptado))); 
    }
